...
- **Timeout Mismatch Detector (Phase 15)** — integrated into the CDK Preflight Report.
- **AWS Service Map** — new command `AWS: Generate Service Map` reads every `*.template.json` in `cdk.out/`, resolves CloudFormation `Ref` and `Fn::GetAtt` cross-resource references, and opens an interactive React Flow panel. Nodes are auto-arranged left-to-right by service type: Cognito → API Gateway → AppSync → DataSources → Lambda → DynamoDB → SQS/SNS → EventBridge. Detected connections: AppSync DataSource → Lambda (`invokes`), AppSync DataSource → DynamoDB (`reads/writes`), Cognito → AppSync (`auth`), SQS/SNS → Lambda via EventSourceMapping (`triggers`), Lambda env var refs to DynamoDB/SQS (`env ref`), EventBridge rule targets (`triggers`). Multiple CDK stacks shown as tabs. Animated edges for `triggers` and `invokes`. MiniMap, Controls, and drag/zoom supported. Requires internet to load React + React Flow via CDN. Detects API Gateway integration timeout shorter than Lambda timeout (504 risk), AppSync Lambda data source exceeding the 30s resolver limit, SQS visibility timeout shorter than Lambda timeout (duplicate processing risk), and Lambda functions configured at the 900s maximum.
